...
def get_stock_data(stcok_code, down_stock_list):
    """
    获取历史行情数据
...
class CrawlerThread(threading.Thread):
    ...
    def run(self):
        while not q_codes.empty():
            code = q_codes.get()
            results = get_stock_data(code, self.down_stock_list)
...
class CollegeTreeListCtrl(wx.gizmos.TreeListCtrl):
    ...
    def refDataShow(self, newDatas, down_stock_list):
...
class DataPanel(MainPanel):
    ...
    def init_tree(self, parent):
        ...
        self.treeListCtrl.refDataShow(self.stock_tree_info, self.down_stock_list)  # TreeCtrl显示数据接口
    ...
    def add_patten_para_lay(self, sub_panel):
        ...
        self.patten_end_time = DatePickerCtrl(sub_panel, -1,
                                                     style=wx.adv.DP_DROPDOWN | wx.adv.DP_SHOWCENTURY | wx.adv.DP_ALLOWNONE)  # 结束时间
        self.patten_start_time = DatePickerCtrl(sub_panel, -1,
            ...
            if patten_recognize.ShowModal() == wx.ID_OK:
                ...
                for stcok_code in down_stock_list:
                    ...
                    try:
                        ...
                        stock_dat = CsvBackend.load_history_st_data(data_path + stcok_code + '.csv',
            ...
            if rpstop_recognize.ShowModal() == wx.ID_OK:
                ...
                for name, code in code_name.items():
                    ...
                    try:
                        ...
                        stock_dat = CsvBackend.load_history_st_data(data_path + sym + num + '.csv',
                                                                    sdate_val, edate_val, st_auth)
