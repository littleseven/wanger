...
from common.DatHandle import day_down_resample, \
    day_to_qfq, \
    day_to_hfq
...
class CsvBackend():
    ...
    def __init__(self, src):
        ...
        if self.src == "离线每日指标":
            ...
            self.tran_col = {"股票代码": "股票代码", "股票名称": "股票名称", "报告日期": "报告日期", "基金家数": "基金家数", "占流通盘比率": "占流通盘比率",
                             "基金数量与上期相比": "基金数量与上期相比", "基金持股数（万股）": "基金持股数（万股）", "持股数与上期相比": "持股数与上期相比",
                             "基金持股市值": "基金持股市值",
    ...
    def load_stock_data(path, sdate, edate, adjust_val='不复权', period_val='周线'):
        ...
        csv_df = pd.read_csv(path, parse_dates=True, index_col=1, encoding='gbk', engine='python', iterator=True,
                             chunksize=5000)
