...
from graph.BaseGraphs import MplTypesDraw, MethodTable


# 属于<8.1 定制可视化接口>的实现
class MarketGraphIf(MplTypesDraw):
    ...
    # 参考<8.2.1 原生量价指标可视化>
    @app.method(u"ochl")
    def ochl_graph(stock_dat, sub_graph, df_dat=None):  # prepare datautil
        type_dict = {'Open': stock_dat.Open,
                     'Close': stock_dat.Close,
                     'High': stock_dat.High,
                     'Low': stock_dat.Low
                     }
        view_function = MplTypesDraw.mpl.get_method(u"ochl")
        view_function(stock_dat.index, type_dict, sub_graph)

    # 参考<8.2.1 原生量价指标可视化>
    @app.method(u"vol")
    def vol_graph(stock_dat, sub_graph, df_dat=None):  # prepare datautil
        ...
        view_function = MplTypesDraw.mpl.get_method(u"bar")
        view_function(stock_dat.index, type_dict, sub_graph)

    # 参考<8.2.2 移动平均线 SMA 可视化>
    @app.method(u"sma")
    def sma_graph(stock_dat, sub_graph, periods):  # prepare datautil
        for val in periods:
            type_dict = {'SMA'+str(val): stock_dat.Close.rolling(window=val).mean()}
            view_function = MplTypesDraw.mpl.get_method(u"line")
            view_function(stock_dat.index, type_dict, sub_graph)

    # 参考<8.2.4 趋势类指标 MACD 可视化>
    @app.method(u"macd")
    def macd_graph(stock_dat, sub_graph, df_dat=None):  # prepare datautil
    ...
    @app.method(u"kdj")
    def kdj_graph(stock_dat, sub_graph, df_dat=None):  # prepare datautil
    ...
    # 参考<8.3.1 均线交叉信号可视化>
    @app.method(u"cross")
    def cross_graph(stock_dat, sub_graph, df_dat=None):  # prepare datautil
        ...
        view_function = MplTypesDraw.mpl.get_method(u"annotate")
        view_function(stock_dat.index, type_dict, sub_graph)
    ...
    # 参考<8.3.2 股价跳空缺口可视化>
    @app.method(u"jump")
    def jump_graph(stock_dat, sub_graph, df_dat=None):  # prepare datautil
    ...
    # 8.3.4 黄金分割与支撑/阻力线
    @app.method(u"fibonacci")
    def fibonacci_graph(stock_dat, sub_graph, df_dat=None):  # prepare datautil
        # 绘制黄金分割线
        ...
        view_function = MplTypesDraw.mpl.get_method(u"hline")
        view_function(stock_dat.index, type_dict, sub_graph)


    ##### K线形态指标
    # 参考<8.4.2 常见 K 线形态的识别方法>
    @app.method(u"CDLDARKCLOUDCOVER")
    def dark_cloud_cover_graph(stock_dat, sub_graph, df_dat=None):  # prepare datautil
        # 绘制 talib K线形态 乌云压顶
    ...
    @app.method(u"CDL3BLACKCROWS")
    def balck3_crows_graph(stock_dat, sub_graph, df_dat=None):  # prepare datautil
        # 绘制 talib K线形态 三只乌鸦
    ...
    # 参考<8.4.2 常见 K 线形态的识别方法>
    @app.method(u"CDLDOJISTAR")
    def do_jistar_graph(stock_dat, sub_graph, df_dat=None):  # prepare datautil
        # 绘制 talib K线形态 十字星
    ...
    # 参考<8.4.2 常见 K 线形态的识别方法>
    @app.method(u"CDLHAMMER")
    def hammer_graph(stock_dat, sub_graph, df_dat=None):  # prepare datautil
        # 绘制 talib K线形态 锤头
    ...
    # 参考<8.4.2 常见 K 线形态的识别方法>
    @app.method(u"CDLSHOOTINGSTAR")
    def shooting_star_graph(stock_dat, sub_graph, df_dat=None):  # prepare datautil
        # 绘制 talib K线形态 射击之星
