...
from graph.BaseGraphs import MethodTable, MplTypesDraw


class BacktestingGraphIf(MplTypesDraw):

    app = MethodTable()

    def __init__(self, **kwargs):
        MplTypesDraw.__init__(self)
        self.fig = plt.figure(figsize=kwargs['figsize'], dpi=None, facecolor="white")#创建fig对象
        print(matplotlib.pyplot.Figure.get_size_inches(self.fig))
        print(matplotlib.pyplot.Figure.get_dpi(self.fig))
        self.graph_dict = {}
        self.graph_curr = []
    ...
    @app.method(u"cash_profit")# cash profit and retracement
    ...
    @app.method(u"cmp_profit")  # relative_profit
    ...
    # 参照<9.1.5 度量策略的最大风险回撤>
    @app.method(u"close_retrace")  # relative_profit
    ...
    def trade_graph(stock_dat, sub_graph, para_dat):
        ...
        view_function = MplTypesDraw.mpl.get_method(u"filltrade")
        view_function(stock_dat.index, type_dict, sub_graph)
